...
# reaches the database. Who may make each move is decided separately, by role,
# in `app.services.incidents`.
ALLOWED_TRANSITIONS: dict[IncidentStatus, set[IncidentStatus]] = {
    # Open can be closed directly, for a report withdrawn before work starts.
    IncidentStatus.OPEN: {IncidentStatus.IN_PROGRESS, IncidentStatus.CLOSED},
    # In progress can go back to open; the assignee is kept, only the status
    # changes. Only an administrator or the assigned engineer can do this.
    ...
    # Resolved goes back to in progress when the reporter says the fix did
    # not work; closing is the reporter's confirmation that it did.
...
class IncidentUpdate(ApiModel):
    ...
    # The service requires these three together when any one is sent, so a
    # partial move cannot pair a floor with the wrong building.
...
class IncidentStatusChange(ApiModel):
    """A move to a new status.

    Whether the move is allowed at all is checked in the service against
...
class IncidentPriorityChange(ApiModel):
    """Administrator changes priority. The optional reason goes on the
    timeline."""
...
class EscalationDecision(ApiModel):
    ...
    `note` is recorded on the timeline entry for the decision; the reporter's
    original escalation reason is kept on the incident either way.
    """
...
class SimilarIncidentQuery(ApiModel):
    ...
    "Open" here means not yet resolved or closed. Every location field given
    must match, so a seat narrows further than a building.
    """
    ...
    def needs_one_location(self) -> "SimilarIncidentQuery":
        # Without a location the query would match every open incident in
        # the category, which is not "the same place".
...
class IncidentDetail(ApiResponse):
    ...
    def is_open(self) -> bool:
        """Still being worked on: anything short of resolved or closed."""
        return self.status not in (IncidentStatus.RESOLVED, IncidentStatus.CLOSED)

    @property
    def next_statuses(self) -> list[IncidentStatus]:
        """The statuses this incident may move to next, from the transition
        table.

        Intended to drive the workflow diagram and the status dropdown. Note it
        is a plain property, not a computed field, so it is not included in the
        serialised response; a client must derive the same set itself."""
...
class IncidentTimeline(ApiResponse):
    """Every recorded change to an incident, oldest first."""
