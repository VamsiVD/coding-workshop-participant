...
class EngineerCreate(ApiModel):
    ...
    # Same domain rule as self-registration; the users_email_domain CHECK
    # would refuse anything else, this just says so readably. Note there is no
    # letters-and-numbers password rule here, unlike RegisterRequest.
...
class EngineerUpdate(ApiModel):
    """Administrator edit. Every field optional; omitted fields are left alone.

    Deactivating the account is `DELETE /engineers/{user_id}`, not a field
    here, because it is refused while the engineer still holds tickets.
    """
...
class EngineerOut(ApiResponse):
    """An engineer profile joined with its user account. Keyed by `user_id`,
    since the profile has no id of its own."""
...
class EngineerWorkload(ApiResponse):
    ...
    Note: no route currently uses this model; the workload report returns
    `EngineerWorkloadRow` from `app.schemas.reports`. Its properties are plain
    Python properties, so they would not be serialised in a response."""
    ...
    def active_tickets(self) -> int:
        """Everything not yet resolved or closed, matching the repository's
        `active_ticket_count`."""
