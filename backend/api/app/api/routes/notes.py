...
HTTP layer only: the rules live in `app.services.notes`. Access follows the
incident, so a caller who cannot see the incident gets 404 for its notes too.
"""
...
def list_notes(
    ...
    """A page of the incident's notes. Anyone who can see the incident."""
...
def create_note(
    ...
    400 once the incident is closed; reopen it to continue the conversation.
    """
...
def update_note(
    ...
    403 for anyone else who can see the incident.
    """
...
def delete_note(note_id: int, conn: DbConnection, user: CurrentUserDep):
    """The author, or an administrator. 403 for anyone else who can see the
    incident."""
