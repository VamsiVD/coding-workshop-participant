...
incident does, by cascade. The incidents and notes services write here
alongside each change (routes -> services -> repositories).
"""
...
def timeline(conn: Connection, incident_id: int) -> list[dict]:
    ...
    The users join is LEFT because `actor_id` is nulled when a user row is
    removed; such events come back with `actor` set to None.
    """
    with conn.cursor() as cur:
        cur.execute(
            """
        ...
        # Fold the flat actor_* columns into a nested `actor` object, the
        # same shape the notes repository gives its authors.
        rows = []
