...
```bash
docker compose exec -T postgres psql -U acme -d acme_incidents -q < backend/db/seed_dummy.sql
```
...
```bash
docker compose exec -T postgres psql -U acme -d acme_incidents -q < backend/db/smoke.sql
```
