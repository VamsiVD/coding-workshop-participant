// "Around you" section of the employee dashboard: open incidents other people
// reported near the user, with an "Affects me too" toggle so they need not
// file a duplicate.
...
export default function NearbyIncidents({ items, locationLabel, onToggle }) {
  // [CONCEPT: Conditional rendering] Hide the whole section when there is nothing nearby (items may also be undefined).
  ...
  return (
    <Box component="section" sx={{ border: `1px solid ${admin.line}`, borderRadius: '12px', p: '18px 20px', display: 'grid', gap: 1.5 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 260px), 1fr))', gap: 1.5 }}>
        {/* [CONCEPT: List rendering and keys] One card per incident, keyed by its ref. */}
        {items.map((n) => (
          <Box key={n.ref} sx={{ bgcolor: admin.surface, border: `1px solid ${admin.line}`, borderRadius: '10px', p: '12px 14px', display: 'grid', gap: 1 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 1, alignItems: 'center' }}>
              ...
              {/* [CONCEPT: Component composition] Reuses the shared StatusChip in its small size. */}
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 1, fontSize: 12.5, color: admin.muted }}>
              ...
              {/* [CONCEPT: Accessibility] A toggle button: aria-pressed reports the on/off state, and onToggle sends the flipped value up. */}
              <Button
                size="small"
